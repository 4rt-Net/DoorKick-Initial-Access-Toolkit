...
class HadoopYARNModule(BaseModule):
    ...
    def check_yarn_resourcemanager(self, target, module_name):
        ...
        try:
            req = self.stealth_request(info_url, method="GET")
        ...
        try:
            req = self.stealth_request(apps_url, method="GET")
            req.add_header("Accept", "application/json")
            response = urllib.request.urlopen(req, timeout=5)
    ...
    def test_new_application_api(self, target, base, module_name):
        ...
        try:
            req = self.stealth_request(new_app_url, method="POST")
            req.add_header("Accept", "application/json")
            response = urllib.request.urlopen(req, timeout=5)
